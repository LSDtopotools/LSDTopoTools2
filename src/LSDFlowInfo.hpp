...
class LSDFlowInfo
{
  public:
    ...
    /// @brief Get the row and column indices of a given node.
    /// @param current_nodes vector of node indices
    /// @param curr_rows Empty integer vector to be assigned the row index of the given
    ///  nodes.
    /// @param curr_cols Empty integer vector to be assigned the column index of the given
    ///  nodes.
    /// @author SMM
    /// @date 15/06/22
    void retrieve_rows_and_cols_from_node_list(vector<int> current_nodes,vector<int>& curr_rows,
    ...
    /// @brief Get the number of contributing pixels from a list of nodes.
    /// @param current_nodes vector of node indices
    /// @return a vector containing the number of contributing pixels
    /// @author SMM
    /// @date 15/06/22
    vector<int> retrieve_contributing_pixels_from_node_list(vector<int> current_nodes);
    ...
    ///  to get a discharge raster. This has been superceded by v2 and is only here for 
